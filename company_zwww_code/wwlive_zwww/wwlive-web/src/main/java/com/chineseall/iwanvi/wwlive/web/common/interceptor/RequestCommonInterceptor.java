...
public class RequestCommonInterceptor implements HandlerInterceptor {
	...
	static final Logger LOGGER = Logger
			.getLogger(RequestCommonInterceptor.class);

	private static String loginUri = "/app/user/login";

	private static String indexUri = "/wwlive/index";
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if (uri.equals(loginUri) || uri.equals(loginUri + ".json")) {
			if (isLostParams(coverKey, nonce, requestId)) {
				writeResult(response);
				return false;
			}
		} else {//非登录
			if (uri.startsWith(external) || uri.equals(indexUri)) {// 如果为external跳过检查 假官网也跳过
				return true;
			}
