...
;(function(win,$){
    ...
    sS.prototype.init = function(options){
		...
		if (typeof options.touchcb == "function") {
				options.touchcb()
			};
            fnTouches(e);
            fnTouchstart(e, me);
    ...
    function fnLazyLoad(me, index){
        var $li = me.opts.ul.children();
        var $thisImg = $li.eq(index).find('[data-original]');
        if($thisImg){
            $thisImg.each(function(i){
                var $this = $(this);
                if($this.is('img')){
                    $this.attr('src',$this.data('data-original'));
                    $this.removeAttr('data-original');
                }else{
                    $this.css({'background-image':'url('+$this.data('data-original')+')'});
                    $this.removeAttr('data-original');
                }
            });
        }
    ...
    function fnTouchstart(e, me){
		var json={"data":{"action":"down"},"fun":"touchInAdSlider"};
        window.stub.jsControlClient(JSON.stringify(json));
    ...
    function fnTouchmove(e, me){
        ...
        if(!me.opts.continuousScroll){
            ...
            if(me._index == 0 && me._moveDistance > 0 || (me._index + 1) >= me._liLength && me._moveDistance < 0){
                me._moveDistance = 0;
                if (typeof me.opts.slcb == "function"){me.opts.slcb()}
            }
        }
        // 触摸时跟手滚动
    ...
    function fnTouchend(me){
		var json={"data":{"action":"up"},"fun":"touchInAdSlider"};
            window.stub.jsControlClient(JSON.stringify(json));
        // 优化触摸禁止事件
        if(!me.isScrolling){
            fnAutoSlide(me);
